...
class OrderController extends Controller
{
    ...
    public function index(Request $request, $customerId = null)
    {
        ...
        // $products = Product::select('id', 'name', 'price', 'stock_quantity')->toArray(); // 50 products per page
            ...
            $products = Product::select('id', 'name', 'price', 'stock_quantity','weight')->get()->keyBy('id')->toArray();
            // dd(count($products));
    ...
    public function ajaxList(Request $request)
    {
        ...
        $query = Product::select('id', 'name', 'price', 'stock_quantity','weight');
    ...
    public function getOrders(Request $request)
    {
      ...
      if ($request->filled('selectedDateRange')) {
            ...
            if (count($dates) === 2) {
                ...
                // 1. Get the Start Date: Start of the day (00:00:00)
                $startDateTime = \Carbon\Carbon::createFromFormat($inputFormat, $dates[0])->startOfDay();
                
                // 2. Get the End Date: End of the day (23:59:59)
                // This ensures all records for the selected end day are included.
                $endDateTime = \Carbon\Carbon::createFromFormat($inputFormat, $dates[1])->endOfDay();
                
                // Use whereBetween on the datetime column
                $query->whereBetween('order_date', [$startDateTime, $endDateTime]);
        
            } elseif (count($dates) === 1) {
                // Handle single date selection, covering the full day
                $singleDateStart = \Carbon\Carbon::createFromFormat($inputFormat, $dates[0])->startOfDay();
                $singleDateEnd = \Carbon\Carbon::createFromFormat($inputFormat, $dates[0])->endOfDay();
                
                $query->whereBetween('order_date', [$singleDateStart, $singleDateEnd]);
            }
        }
    ...
    public function uploadProductImage(Request $request)
    {
     ...
     public function getCustomer($name){
        try{
            $customer = UserCustomers::where('name', $name)->first();
    ...
    */
    public function searchCustomers(Request $request)
    {
        ...
        $customers = UserCustomers::select('id', 'name', 'email', 'phone')
    ...
    {
        $orders = Order::where('user_customer_id', $customerId)
    ...
    public function bulkDeleteOrders(Request $request)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => count($request->order_ids) . ' order(s) deleted successfully and stock restored.'
            ]);
        } catch (\Exception $e) {
            DB::rollback();
